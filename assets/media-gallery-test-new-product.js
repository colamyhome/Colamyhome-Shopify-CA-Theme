    ...
    class MediaGallery extends HTMLElement {
      constructor() {
        ...
        this.selectors = {
          ...
          thumbsNavPrev: '[data-media-thumbs-prev]',
          thumbsNavNext: '[data-media-thumbs-next]',
        ...
        this.thumbnailNavigationBound = false;
      }
  ...
  breakpoints: {
    768: {
      ...
      bindThumbnailNavigation() {
        ...
        if (!thumbsSlider || this.thumbnailNavigationBound) return;

        const { thumbsNavPrev, thumbsNavNext } = this.elements;
        if (!thumbsNavPrev && !thumbsNavNext) return;
        ...
        this.elements.thumbnails?.addEventListener('wheel', this.handleThumbnailWheel.bind(this), { passive: false });

        thumbsSlider.on('afterInit', this.updateThumbnailNavigation.bind(this));
        thumbsSlider.on('slideChange', this.updateThumbnailNavigation.bind(this));
        thumbsSlider.on('reachBeginning', this.updateThumbnailNavigation.bind(this));
        thumbsSlider.on('reachEnd', this.updateThumbnailNavigation.bind(this));
        thumbsSlider.on('fromEdge', this.updateThumbnailNavigation.bind(this));

        this.thumbnailNavigationBound = true;
        this.updateThumbnailNavigation();
      }

      handleThumbnailWheel(event) {
        if (this.mediaLayout !== 'vertical-carousel' || FoxTheme.config.mqlMobile) return;

        const thumbsSlider = this.thumbsInstance?.slider;
        if (!thumbsSlider || Math.abs(event.deltaY) <= Math.abs(event.deltaX)) return;

        const isScrollingDown = event.deltaY > 0;
        const canMove = isScrollingDown ? !thumbsSlider.isEnd : !thumbsSlider.isBeginning;
        if (!canMove) return;
      ...
      updateThumbnailNavigation() {
        ...
        this.elements.thumbsNavPrev?.toggleAttribute('disabled', thumbsSlider.isBeginning);
        this.elements.thumbsNavNext?.toggleAttribute('disabled', thumbsSlider.isEnd);
      }
