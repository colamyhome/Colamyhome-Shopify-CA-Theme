  ...
  customElements.define(
    ...
    class CountdownTimer extends HTMLElement {
      ...
      init() {
        ...
        this.shouldSplitDigits = this.dataset.splitDigits === 'true';
      ...
      updateDisplay(remainingTime) {
        ...
        this.units.forEach((unit) => {
          if (this.elements[unit]) {
            this.renderTimeComponent(this.elements[unit], timeComponents[unit]);
          }
        });
      }

      renderTimeComponent(element, value) {
        const formattedValue = this.formatTimeComponent(value);
        if (!this.shouldSplitDigits) {
          element.textContent = formattedValue;
          return;
        }

        const digits = formattedValue.split('').map((digit) => {
          const digitElement = document.createElement('span');
          digitElement.className = 'countdown-timer__digit';
      ...
      resetCountdown() {
        ...
        this.units.forEach((unit) => {
          if (this.elements[unit]) {
            this.renderTimeComponent(this.elements[unit], 0);
          }
        });
      }
