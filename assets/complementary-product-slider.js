  ...
  customElements.define(
    ...
    class ComplementaryProductSlider extends HTMLElement {
      constructor() {
        ...
        this.bindQuickViewTriggers();
        this.init();
      }

      bindQuickViewTriggers() {
        if (this.dataset.quickViewBound === 'true') return;
        this.dataset.quickViewBound = 'true';

        const openQuickView = (event) => {
          const trigger = event.target.closest('[aria-controls^="QuickView-"]');
        ...
        // Click is enough on desktop; pointerup is a fallback on touch devices where Swiper may swallow click.
        this.addEventListener('click', openQuickView);
        this.addEventListener('pointerup', openQuickView);
      }
      ...
      fixQuickviewDuplicate() {
        ...
        Array.from(this.querySelectorAll('quick-view-modal')).forEach((modal) => {
          ...
          const isDuplicateSlide = !!modal.closest('.swiper-slide-duplicate');
          const keptModal = modalMap.get(modalID);
          ...
          const keptIsDuplicateSlide = !!keptModal.closest('.swiper-slide-duplicate');

          // Prefer modal in real slide; remove duplicate-slide modal when both exist.
          if (keptIsDuplicateSlide && !isDuplicateSlide) {
            keptModal.remove();
            modalMap.set(modalID, modal);
            return;
          }
